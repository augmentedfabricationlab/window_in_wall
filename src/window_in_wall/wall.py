...
class Wall(object):
    ...
    def in_gate_tan(self, x_val, z_val, tan_a, tan_b, tan_c):
            """returns True if the relative coordintes (gate_rel_x,gate_rel_z) lie in
            a triangular gate of size gate_size. otherwise false
            """   
            if x_val > - tan_c/tan_b:
                if x_val < math.pi/tan_b - tan_c/tan_b:
                    if z_val > tan_a*math.tan(tan_b*(x_val-tan_c))+self.height/2:
                        return True
            else:
                return False

            if x_val > self.width - math.pi/tan_b + tan_c/tan_b:
                if x_val < self.width + tan_c/tan_b:
                    if z_val < tan_a*math.tan(tan_b*(x_val-tan_c+self.width))+self.height/2:
                        return True
            else:
                return False
